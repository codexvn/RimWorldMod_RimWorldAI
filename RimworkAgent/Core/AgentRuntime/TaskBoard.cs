...
namespace RimworkAgent.Core.AgentRuntime
{
    ...
    public static class TaskBoard
    {
        ...
        {
            lock (_lock) { _tasks.Clear(); _nextId = 1; Save(); }
        }
        ...
        private static void Save()
        {
            ...
            catch (Exception ex) { CoreLog.Error($"[TaskBoard] 保存失败: {ex.Message}"); }
        }
        ...
        private static void Load()
        {
            ...
            try
            {
                ...
                if (data != null) { _tasks = data.tasks ?? new List<TaskEntry>(); _nextId = data.nextId; }
            }
            catch (Exception ex) { CoreLog.Error($"[TaskBoard] 加载失败: {ex.Message}"); _tasks = new List<TaskEntry>(); _nextId = 1; }
        }

        private class SaveData { public List<TaskEntry>? tasks { get; set; } public int nextId { get; set; } }
    }
}
