...
{
    public class Tool_DesignateClearPlants : ITool, IRequiresAdvanceTick
    {
        public string Name => "designate_clear_plants";
        public string Description => "标记指定区域的杂草、灌木等非树木非收获植物进行割除。等效建筑师工具栏\"割除\"命令过滤（CutPlant，排除可收获树木）。砍伐树木/仙人掌请使用 designate_plants_cut，收割作物请使用 designate_harvest。提供 end_x/end_y 可划定矩形范围。坐标范围为闭区间（两端坐标均包含）。";
        ...
        public async Task<ToolResult> ExecuteAsync(JsonElement? args)
        {
            ...
            return await McpCommandQueue.DispatchAsync(() =>
            {
                try
                {
                    ...
                    sb.Append(isRange
                        ? $"已标记清除范围 ({minX},{minZ})~({maxX},{maxZ})：{designated} 株"
                        : $"已标记清除坐标 ({posX}, {posY})：{designated} 株");
