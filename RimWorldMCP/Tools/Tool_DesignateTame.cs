    ...
    public class Tool_DesignateTame : ITool, IRequiresAdvanceTick
    {
        ...
        public async Task<ToolResult> ExecuteAsync(JsonElement? args)
        {
            ...
            return await McpCommandQueue.DispatchAsync(() =>
            {
                ...
                for (int x = minX; x <= maxX; x++)
                    for (int z = minZ; z <= maxZ; z++)
                    {
                        ...
                        foreach (var t in cell.GetThingList(map))
                        {
                            ...
                            if (!TameUtility.CanTame(pawn)) continue;
                            if (pawn.health.hediffSet.HasHediff(HediffDefOf.Scaria)) continue;
                            if (map.designationManager.DesignationOn(pawn, DesignationDefOf.Tame) != null) continue;
                ...
                if (count == 0) return ToolResult.Success("区域内没有可驯服的动物。");
                return ToolResult.Success($"已标记 {count} 个驯服目标: {string.Join(", ", names)}");
            });
        }
