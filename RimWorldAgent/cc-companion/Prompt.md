...
2. **在 Plan 阶段第一时间调用 `get_skills()` 查看可用 Skill**，用 `active_skill` 加载与当前场景相关的 Skill 获取详细战术指导
3. 制定计划（用 task_create 创建任务）
4. 计划制定完成后，调用 `enter_act()` 恢复游戏
5. 在 Act 阶段执行具体操作（调用游戏 Tool）
6. 执行完成后可以再次 `enter_plan()` 进行下一轮规划
...
**进入 PLAN 模式后必须先调用 `get_skills()` 查看可用领域知识**，根据当前场景用 `active_skill` 加载相关 Skill 后再制定计划。
