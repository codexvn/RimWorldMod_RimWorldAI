...
`designate_build` 的 `rotation` 参数控制建筑朝向：
...
**制冷机（Cooler）**：正面 = 冷端（吹冷风），背面 = 热端（排热气）。
- 放在墙上：冷端必须朝向房间内部，热端朝向室外或无屋顶区
- 南墙放制冷机 → `rotation: "North"`（冷端朝北入室，热端朝南排室外）
- 北墙放制冷机 → `rotation: "South"`（冷端朝南入室，热端朝北排室外）
- 西墙放制冷机 → `rotation: "East"`（冷端朝东入室，热端朝西排室外）
- 东墙放制冷机 → `rotation: "West"`（冷端朝西入室，热端朝东排室外）
