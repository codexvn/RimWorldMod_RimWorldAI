...
**第 1 阶段**：
- **最优先：分配武器和护甲装备，然后建造冷冻间**。用 `designate_build(thing_def:"Cooler", rotation:"...")` 建制冷机——蓝色制冷端朝内、红色朝室外，根据所在墙壁选 rotation（见"设备方向"节），目标-15°C 以下。易腐物品放入后冰冻长期保鲜。
...
- `Research` 尽早给智识最高者 1；开局至少保持 1 名专职研究员
...
3. **烹饪食物**
   ...
   - 简单食物 = 1 食材→1 份饭，精致食物 = 1 肉+1 素→1 份饭

4. **检查储备**：用 `get_resources` 查看食物数量，`check_colony` 查看食物警告。保持 5 天以上储备（人均每天约 2 份）

### 房间与建造
- 建造前先画规划草图：plan_add 画区域→plan_list 查看→确认布局→再建造
- **所有房间墙体通过 `designate_room` 或 `apply_base_template` 建造
- **相邻房间必须共用墙壁**：B 的起点 = A 的终点（不是 +1）。例如 Room A: (10,0)-(22,12)，Room B: (22,0)-(34,12)，两间共用 x=22 墙体。闭区间公式 `end - pos + 1 = size`（如 22-10+1=13），推导 `end = pos + size - 1`
...
`designate_build` 的 `rotation` 参数控制建筑朝向。对方向敏感设备（目前仅制冷机 Cooler），`rotation` 指定的是**热端（排热方向）**，冷端朝反方向。通风口 Vent、加热器 Heater 等无方向之分，用默认 `North` 即可。

| rotation | 热端朝向 | 冷端朝向 |
|----------|---------|---------|
| North（默认） | 北 / 地图上方 | 南 / 地图下方 |
| East | 东 / 地图右方 | 西 / 地图左方 |
| South | 南 / 地图下方 | 北 / 地图上方 |
| West | 西 / 地图左方 | 东 / 地图右方 |

> 依据：`Building_Cooler.TickRare` 源码 — `热端 = Position + North.RotatedBy(Rotation)`，`冷端 = Position + South.RotatedBy(Rotation)`。`rotation` 即建筑正面 = 热端方向。

普通建筑（工作台、发电机等）方向不影响功能，使用默认 `North` 即可。

**制冷机（Cooler）**：热端排热气，冷端吹冷风。
- 放在墙上：冷端必须朝向房间内部，热端朝向室外或无屋顶区
- 南墙放制冷机（房间在北）→ `rotation: "South"`（热端朝南排室外，冷端朝北入室）
- 北墙放制冷机（房间在南）→ `rotation: "North"`（热端朝北排室外，冷端朝南入室）
- 西墙放制冷机（房间在东）→ `rotation: "West"`（热端朝西排室外，冷端朝东入室）
- 东墙放制冷机（房间在西）→ `rotation: "East"`（热端朝东排室外，冷端朝西入室）
- 记忆口诀：**rotation = 热端朝外**，冷端自然朝内
- 如不确定朝向，在房间计划中标注"此墙放制冷机，冷端朝内"
- **制冷机不能用 `designate_room` 放置**（无 rotation 参数），必须单独用 `designate_build` 指定 rotation
- 建造后用 `get_device_info` 查看温控组件，用 `temperature_grid` 验证冷热端温度分布

**风力发电机（Wind Turbine）**：正面方向 = 风力区域延伸方向。
- 风力区域为发电机正面延伸的长矩形，区域内不能有树、墙、屋顶等阻挡物
- 两台风力发电机可交错重叠风力区域（不互相阻挡）
- 建造后用 `get_device_info` 查看风力区域范围

### 环境与选址
- **蒸汽喷泉产热量极高**（≈4000W），室内用喷泉发电会导致房间高温（制冷也无法抵消）。喷泉必须露天放置或独立围起（无屋顶），严禁建在居住区/冷库/储藏间内。冰盖等极寒环境可将喷泉围在基地核心区供暖，但仍需通风屋顶
- **轻型地形禁止建家**：沙地、沼泽、浅水等无法支撑重型建筑。建基地前用 `get_tile_detail` 或 `terrain_grid` 查看地表类型，确认能铺设混凝土地面后再建造
...
- **discover_tools 中标记 `【需advance_tick】` 的工具不会立刻生效**：需要用 `advance_tick` 推进游戏时间后殖民者才会执行。调用后直接继续下达指令即可，无需等待结果。
...
- **心情**：保持≥50%；精致食物+娱乐+干净工装；动物尸体搬进冷库→屠宰→烹饪，敌人尸体扔室外 corpse_dump 防心情惩罚
...
参数不确定时先用 get_tool_schema 查询 Schema，参数错误时也会自动返回该命令的完整文档。
...
- `每早汇报` — 游戏已自动暂停。按以下流程执行：
  ...
  5. **进入执行**: 调用 enter_act(reason="晨报计划完成") 进入执行阶段，用 advance_tick 推进游戏时间
