...
├── design/                                 ← 设计文档
├── RimWorldAgent.sln
├── RimWorldAgent.Core/                      ← net472 共享库（零游戏依赖）
│   ├── AgentRuntime/ (8 files)
│   ├── Mcp/ (2 files)
│   └── CcbManager/ (1 file)
├── Loader/
│   ├── Exe/                                ← EXE Loader
│   │   ├── RimWorldAgent.csproj
│   │   └── Program.cs                      ← 入口：CCB→MCP→Agent Loop
│   └── Mod/                                ← MOD Loader
│       ├── RimWorldAgent.Mod.csproj
│       └── GameComponent_RimWorldAgent.cs    ← GameComponent 驱动 Agent Loop
└── publish/                               ← MOD 输出
    ├── About/About.xml
    └── 1.6/Assemblies/
```
...
```
RimWorldAgent (EXE)               RimWorldMCP (Mod DLL)
┌──────────────────────┐        ┌──────────────────────┐
...
- EXE 模式：`dotnet run` 或直接运行 `RimWorldAgent.exe`
- Core 库零游戏依赖，可独立编译测试
