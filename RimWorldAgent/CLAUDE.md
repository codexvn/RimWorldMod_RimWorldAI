...
`CcbManager.StartCore()` 从 `InternalToolRegistry.SkillRegistry` 读取全部 Skill（`name: description`），JSON 序列化后通过环境变量 `CCB_SKILLS` 传入 companion。`session.ts` 从 `CONFIG.skills` 构建 "可用领域知识" 段落，插入 `systemPrompt` 中 `Prompt.md` 之后、`SYSTEM_PROMPT_DYNAMIC_BOUNDARY` **之后**。因此 Skill 变更不会使缓存前缀失效（仅影响动态后缀）。

Skill 加载顺序：`Skills/*.md`（内置，只读）→ `Skills.d/*.md`（用户/AI 创建，可写）。同名时 `Skills.d` 覆盖内置。`create_skill` 和 Mod 设置页管理器只写 `Skills.d`，保存后 `SkillRegistry.Reload()` 立即让 `get_skills` / `active_skill` 生效；system prompt 中的 Skill 列表需 companion 重启后刷新。
