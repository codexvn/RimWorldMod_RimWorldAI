...
| C# → | `abort` | `clear` (bool?) | 中断当前 SDK 会话；`clear=true` 时删除 session-id，不 resume 旧上下文 |
| ← C# | `hello-ok` | (无) | 握手确认 |
...
Skill 加载顺序：`Skills/*.md`（内置，只读）→ `Skills.d/*.md`（用户/AI 创建，可写）。同名时 `Skills.d` 覆盖内置。`create_skill` 和 Mod 设置页管理器只写 `Skills.d`，保存后 `SkillRegistry.Reload()` 立即让 `get_skills` / `active_skill` 生效，无需 companion 重启。Skill 创建/删除不抢占当前会话，仅通过工具结果和一次性 suffix 提醒 Agent。
...
→ WireEvents 分流:
    ...
    │     └─ SendAbort()                      // 运行中才请求结束旧 session
    │   PushUiMessage(User, notifyText)        // UI
    │   RecordUserMessage(notifyText)          // DB
    │   notifyText = Prefix + "\n" + summary + "\n" + Suffix
    │   TickAsync 等 Agent 空闲后 RunAgent(isInterrupted: true)
    │     └─ ContextBuilder 串行发送事件 prompt
    │
    └── Warning / Info / Silent:
...
`InterruptPromptPrefix = "## 事件通知"`, `InterruptPromptSuffix` 为 `AgentOrchestrator` 常量，suffix 内包含事件处理提醒和 Skill 激活提示。
Companion abort 采用**消息缓冲 + generation guard**：abort 触发 `buffering=true`，立即使旧 SDK processor generation 失效，等待旧 processor 停止（最多 2s）后 `startNewSession` 重建 session，回放缓冲消息并返回 `aborted` 确认；旧 generation 的迟到消息直接丢弃。
...
UI 格式：`入 {inputTokens}/{totalPerTurn}({hitRate}%)    Tok {used}/{limit} {pct}% ████░░░░`

**"入"段**（per-turn 本轮值）：
- `inputTokens` = CurrentInputTokens（本轮非缓存输入）
- `totalPerTurn` = CurrentInputTokens + CurrentCacheReadTokens（本轮总输入）
- `hitRate` = CurrentCacheReadTokens / totalPerTurn × 100（本轮缓存命中率）

**"Tok"段**（累计值）：
- `used` = TotalAllTokens（累计总花费 = input + output）
- `limit` = BudgetLimit（预算上限）
- `pct% ██░░` = used / limit × 100，10 段条形图

Per-turn 值由 `SdkMessageParser.message_delta` 从 SDK usage 增量提取。累计值由 `IDbStore` 持久化。Dialog 通过 `ChatStateTypes.UpdateBudget` 渲染。
