...
UI 格式：`入 {perTurnInput}({perTurnHitRate}%) | 总计 {totalAllTokens}/{budgetLimit}({pct}%) ████░░░░`

- **入** = CurrentInputTokens + CurrentCacheReadTokens（本轮实际输入量，含缓存命中）
- **本轮命中率** = CurrentCacheReadTokens / (CurrentInputTokens + CurrentCacheReadTokens) × 100
- **总计** = TotalAllTokens（input + output 累计，用于预算上限检查）
- **进度条** = TotalAllTokens / budgetLimit × 100，10 段彩色条形图

Per-turn 值由 `SdkMessageParser.message_delta` 从 SDK usage 增量提取后写入 `TokenUsageTracker.Current*` 静态属性。累计值由 `IDbStore` 持久化。
