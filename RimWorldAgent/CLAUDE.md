...
```
cacheHitRate = TotalCacheReadTokens / (TotalInputTokens + TotalCacheReadTokens) * 100
```

UI 显示：`入 {input+cacheRead}({hitRate}%) | {budgetUsed}/{budgetLimit}({pct}%)`

- **入** = TotalInputTokens + TotalCacheReadTokens（总输入，含缓存命中）
- **缓存命中率** = TotalCacheReadTokens / (TotalInputTokens + TotalCacheReadTokens) × 100
- **预算** = TotalAllTokens（input + output，用于预算上限检查）
