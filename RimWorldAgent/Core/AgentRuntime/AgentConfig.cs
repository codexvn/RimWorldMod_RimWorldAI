...
4. 可用 `switch_agent(role)` 切换到其他 Agent",
        ...
        {
            Name = "economy", IntervalGameHours = 6, TriggerDaily = true,
            SystemPrompt = @"你是 RimWorld 殖民地的生产与建造经理 (Economy)。
职责: 执行 Overseer 优先级，管理生产/建造/物流/军械。
...
3. 用 `advise_agent(""medic"", advice)` 给医疗官建议伤员处理",
        ...
        {
            Name = "medic", IntervalGameHours = 0, TriggerDaily = true, TriggerOnCombatEnd = true, TriggerOnL3Health = true,
...
## 工作流程
1. 收到医疗事件时，先调用 `enter_plan()` 暂停游戏，评估伤情和资源
