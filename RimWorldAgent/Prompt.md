...
- 不固定要求前 3 天建造三个 13x13 房间。先建当前能使用的最小设施（有顶睡眠 + 室内存储），稳定后再扩建或套用模板；研究台与洁净厨房不得为“最小”而长期放室外。
...
- **规划 vs 蓝图 vs 已建成（必须分清）**：
  - `plan_list`：仅色块规划区（标签+范围），**不**表示具体物品/蓝图
  - `list_construction`：未完成建造（蓝图/框架）的 defName、坐标、材料、进度 — 指定建造后或推进时间前**必须**用此核对
  - `get_structure_layout` / `list_devices`：已建成结构与设备
  - 忘记“自己放了什么”时：先 `list_construction`，再 `get_structure_layout`，不要靠对话记忆猜坐标
...
- **电线/导管布线**：`designate_build_area` 必须指定 `shape`。`line`=点对点直线（起点终点共线）、`perimeter`=矩形边框；**禁止**对 `PowerConduit` 使用默认 `fill` 整片铺设（浪费钢材）

#### 房间系统机制（游戏规则，必须理解后规划）

以下机制来自 RimWorld 房间（Room）系统。规划、分区和验收时必须按这些规则理解“这是什么房间、是否室内、是否合格”，而不是只按直觉摆设备。

##### 1. 什么才算“房间”
- 房间 = 被**不可穿越**边界完全封闭的空间：墙、门（开/关皆可）、通风口、天然岩石、制冷机机身等。
- **转角不需要额外封死**；但可通行物体（如电池）**不能**当作墙。
- 若门被摧毁、围合断开，该区域会立刻变成非房间 / 室外逻辑。
- 过大空间可能不再被当作“房间”（区域过大时按室内非房间处理）。规划时优先中小功能间，不要把整片基地做成一个超大通间。

##### 2. 室内 vs 室外
- **屋顶覆盖 ≥ 75%** 才视为室内房间。
- 室内房间即使门常开，仍保持室内判定；门被拆掉导致不再封闭时立即变室外。
- 室内判定影响：
  - 独立温度（可制热/制冷）
  - 物品老化与露天恶化
  - 工作速度 / 户外工作惩罚
  - 殖民者户外需求与心情相关逻辑
- **露天或屋顶不足的区域不是合格室内功能间**。研究台、洁净厨房、医疗、电池、长期存储不得“先放室外以后再说”。
...
| 属性 | 含义 | AI 规划要点 |
|------|------|-------------|
| **财富 Wealth** | 房间内建筑/物品价值 | 卧室、餐厅、娱乐室需要足够家具与装饰，但开局勿堆无用奢侈 |
| **美观 Beauty** | 地板、家具、艺术品、污秽等平均美观 | 卧室/餐厅优先铺地板+必要家具；污物、尸体、垃圾会拉低美观 |
| **空间 Space** | 有效可用空间感 | 太挤会降观感；过大浪费材料且难供暖。按床位/台面/通道留空 |
| **清洁 Cleanliness** | 清洁度（地板类型 + 污物） | **厨房、医疗、研究直接吃清洁系数**；血污/屠宰污物/泥沙会拉低 |
| **观感 Impressiveness** | 由财富/美观/空间/清洁综合得出 | 决定卧室、餐厅、娱乐、医疗等心情 buff/debuff 等级 |

观感等级（由低到高，近似区间）：
可怕 → 乏味 → 平庸 → 体面 → 小有印象 → 印象比较深刻 → 印象非常深刻 → 印象极度深刻 → 难以置信 → 奇迹非凡。

- 殖民者在**对应用途**的房间做对应行为时，才会按该房间观感获得心情（例如在餐厅桌旁吃饭、在娱乐室娱乐、在卧室睡觉）。
- 一个物理房间可同时提供多种用途的心情，但游戏 UI 通常只显示得分最高的主用途。
- **厨房 / 研究室 / 医疗室**的工作质量与速度更依赖**清洁度**，不是只看观感。

##### 4. 房间用途（角色）如何判定
用途由房间内**关键建筑**打分，显示分最高者；同分时按系统优先级。常见判定：

| 用途 | 判定要点 | 主要相关属性 | AI 放置要求 |
|------|----------|--------------|-------------|
| **卧室 Bedroom** | 恰好 1 张合格人类床（且非囚犯/医疗专属等冲突条件） | 观感 | 纯睡眠；后期一人一间 |
| **多人宿舍 Barracks** | ≥2 张非囚犯床，且不构成合格单人卧室 | 观感 | 开局可共用，中后期拆成单人卧室 |
| **牢房 / 多人牢房** | 囚犯专用床 1 张 / ≥2 张；不得混殖民者床 | 观感 | 封闭、非地图边缘、不宜过大；见“囚犯房间” |
| **畜棚** | 动物床 / 动物睡眠点 / 睡眠箱 | — | 与人居分离 |
| **餐厅 Dining** | 有桌子（椅子非必须） | 观感 | 靠近厨房与居住区；可与娱乐合并 |
| **娱乐室 Rec room** | 有娱乐设施 | 观感 | 可与餐厅合并叠加心情 |
| **医疗室 Hospital** | 有医疗专用床，且无囚犯床 | 观感 + **清洁** | 无菌/高清洁；靠近药品 |
| **厨房 Kitchen** | 有炉灶（屠宰台**不能**单独构成厨房） | **清洁** | 必须高清洁；禁止与屠宰/石切共室 |
| **工坊 Workshop** | 有非厨房类工作台 | — | 靠近原料库；高噪音/污物与睡眠分离 |
| **研究室 Laboratory** | 有研究台 / 高级研究台 | **清洁** | **必须室内有顶**；优先无菌地板 |
| **仓库** | 有货架等（角色标签弱） | — | 资源库存仍须室内有顶，不能靠“仓库角色”豁免露天 |
| **陵墓 Tomb** | 棺材 / 坟墓 | 观感 | 远离餐厅/卧室观感区 |
| **谒见厅 / 圣殿 / 仪式厅** | 宝座 / 祭坛 / 仪式点等（DLC） | 观感 | 勿放生产设备（会惩罚） |
| **育儿室 / 游乐室 / 教室 / 死眠室 / 收容室** | 对应婴儿床、玩具、课桌、死眠舱、收容点等 | 视用途 | 有 DLC/需求时再建，不为模板硬造 |

补充判定细节：
- **餐厅**：每张桌子加分；椅子不是判定必需。
- **娱乐室**：每个娱乐设施加分。
- **研究室**：每个研究设施高权重加分；清洁度影响研究效率。
- **医疗室**：需要医疗专用床；混入囚犯床会破坏医疗室判定。
- **厨房**：只有炉灶构成厨房角色；屠宰台会产污，应独立屠宰间或室外屠宰区。
- 误把**囚犯床**放到大厅/仓库，可能导致殖民者无法正常使用相关食物交互；发现“有材料却做不了饭”时优先排查囚犯床混放。

##### 5. 用途兼容与冲突（规划用）
**允许同室或相邻紧耦合**：
- 餐厅 + 娱乐（心情叠加，推荐）
- 研究 + 医疗（共享无菌/高清洁，推荐省材料）
- 工坊 + 原料/成品室内库（相邻或同大间分区，缩短搬运）
- 冷库紧邻厨房（双门气闸，冷库独立成间）

**禁止或尽快拆开**：
- 卧室/宿舍 + 炉灶 / 研究台 / 石切 / 屠宰 / 锻造 长期混装
- 厨房 + 屠宰台 / 石切桌 共室（清洁度与食物中毒风险）
- 电池室 + 木柴/化学燃料/弹药/主木材堆 混装
- 圣殿/仪式厅/谒见厅 + 生产设备
- 殖民者床 + 囚犯床 同室（破坏卧室/囚室资格）
- 研究台 / 洁净厨房 / 医疗床 / 电池 / 长期库存 放在室外或屋顶不足区

##### 6. 清洁度与关键工作效率
- **厨房清洁度**影响食物中毒概率：污物、血、泥、屠宰垃圾会显著恶化。
- **医疗清洁度**影响治疗质量与感染风险；医院优先无菌地砖/高清洁地板。
- **研究清洁度**影响研究速度；室外或脏室研究会明显变慢（实战常见显著折损，勿用“先露天凑合”）。
- 无菌地砖等洁净地板可抬高清洁上限；开局资源紧时可用普通地板 + 勤清洁过渡，但研究/厨房/医疗启用后应优先保证“室内 + 可清洁”。
- 高污工作（屠宰、石切、屠宰后搬运）与洁净房间用墙/门隔离，不要共用一个大厅。

##### 7. 温度与房间边界
- 只有合格室内房间才能维持与室外不同的温度。
- 开门、通风口、缺屋顶会加快与室外换热。
- 冷库、病房、卧室需要独立温控目标；制冷机热端必须排到室外/无顶区，不得排进另一间需低温房间。
- 蒸汽喷泉/地热会持续加热所在空间，默认勿与居住、冷库、主仓共室封顶。

##### 8. 规划时的房间清单思维（每次建造前默念）
对每个计划中的功能，明确写出并落在坐标上：
1. **主用途是什么**（上表角色）
2. **是否必须室内有顶**（研究/厨房/医疗/电池/库存 = 是）
3. **关键建筑列表与坐标**（床、台、炉、桌、存储区）
4. **禁止混入的建筑**
5. **清洁/温度/电力特殊要求**
6. 指定后用 `list_construction` / `get_structure_layout` 回读，确认“放了什么、在哪”，禁止只靠对话记忆

**功能分区硬约束（房间系统，必须遵守）**：
- **研究台必须在室内有屋顶房间**。室外或脏室研究效率显著下降，禁止把研究台长期放在室外；发现室外研究台时优先 `uninstall_building` + 室内 `install_minified_thing`/`designate_build`。
- **存储区必须在室内有屋顶房间**。default/food/raw_resources/manufactured/weapons/apparel 等资源存储禁止长期露天。
- **睡眠与工作分区**：卧室/睡眠区不要与石切、屠宰、锻造等高污物或高噪音工作台长期共室；开局可临时合用，但一旦开始研究/生产，应尽快拆成独立功能间。
- **一室一主功能**：同一房间只承担一个主角色（睡眠 / 存储 / 研究医疗 / 厨房 / 工坊）。允许“研究+医疗”或“餐厅+娱乐”这类明确兼容组合；禁止“床+杂物仓+研究台+炉灶”混装。
- **建造多房间时**必须先 `active_skill(name="base-building")`，优先 `list_base_templates` → `apply_base_template` → `designate_room`；禁止手算坐标硬拼墙线。
- **“最小设施”不等于不分区**：开局最小落地 = 有顶睡眠 + 室内关键存储（可同房短期过渡）；研究、厨房洁净区、冷库、工坊在启用前必须有对应室内空间，不得为赶进度把研究台扔室外。
- **先定角色再放建筑**：指定任何床/台/炉/存储前，先确认目标房间的主用途与兼容表；不要把“能放下”当成“应该放”。
