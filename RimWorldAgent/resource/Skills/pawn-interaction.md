...
| `set_prisoner_policy` | 设置囚犯政策 | 支持同化并招募，默认确保典狱工作有人做 |
...
3. set_prisoner_policy(policy="convert_then_recruit") → 同化并招募
   - 仅限倒地敌人
   - 工具默认检查典狱工作，若没人做 Warden 会自动挑选合适殖民者启用
```

### 囚犯政策
```
set_prisoner_policy(target_id=<囚犯ID>, policy="convert_then_recruit")
```
- `convert_then_recruit` — 同化并招募：先转换为玩家主要信仰，完成后自动切到劝导并招募
- `recruit` — 劝导并招募：原版典狱流程会先降抵抗，抵抗归零后继续招募
- `reduce_resistance` — 只降低抵抗
- `convert` — 只同化文化/转换信仰
- `maintain_only` / `release` / `execute` — 仅维持 / 释放 / 处决
- `enslave` / `reduce_will` — 奴役 / 降低意志，需要 Ideology DLC
