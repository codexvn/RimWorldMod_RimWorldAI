...
Prompt.md 保存殖民地 Agent 的稳定行为约束。C# 在 initialize 阶段读取，并通过 IPC `AgentRuntimeConfig.prompt` 发送给 Node Host；Node 再通过 ACP session `_meta.systemPrompt` 的 append 字段注入。启动前还会从 SkillRegistry 生成项目目录下的 skills-desc.txt，替换 {skillsTable}。动态世界状态和用户消息仍通过 IPC prompt → ACP session/prompt 发送。Prompt 文件缺失或为空会阻止 Agent session 启动，避免 Agent 在无约束状态下运行。
